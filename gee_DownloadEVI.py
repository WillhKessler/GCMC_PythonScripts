...
def GetImage(bdt, edt, geo, fs, col):
    ...
    composite = ee.Algorithms.Landsat.simpleComposite(
        collection=collection, asFloat=True
    )

    # Add EVI to image. Be sure to change out bands when switching between Landsat collections.
    NDVIcomp = composite.normalizedDifference(colvalues[1][:1]).rename("NDVI")
    EVIcomp = composite.expression(
        "2.5 * ((NIR - RED) / (NIR + 6 * RED - 7.5 * BLUE + 1))",
        {
            "NIR": composite.select(colvalues[1][0]),
            "RED": composite.select(colvalues[1][1]),
            "BLUE": composite.select(colvalues[1][2]),
        },
    ).rename("EVI")
    if fs == 30:
        return EVIcomp
    else:
        texture = EVIcomp.reduceNeighborhood(
    ...
    for h in range(0, len(geolist), 1):
        for i in range(0, len(yrarr), 1):
            for j in range(0, 4, 1):
                ...
                if focalstats[f] == 30:
                    imgndvi = img.select(["EVI"])
                else:
                    imgndvi = img.select(["EVI_mean"])
